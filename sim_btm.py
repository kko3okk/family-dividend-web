...
V=[
 ("S3","九行＋成熟子產業不新進（新基準）",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True)),
 ("B1","＋大盤須在20MA上且20MA上揚",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,idx20=True)),
 ("B2","＋大盤60MA乖離≥3%才進",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,idx_bias_min=3.0)),
 ("B3","＋落底確認：大盤自60日高回落>8%後，須連3日收在20MA上才恢復進場",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,bottom=(8,3))),
 ("B4","＋落底確認（回落>6%、連5日）",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,bottom=(6,5))),
 ("B5","B3＋B2",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,bottom=(8,3),idx_bias_min=3.0)),
 ("B6","B3 且報價位同樣受限",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,bottom=(8,3),r9_block_too=True)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/btm_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/btm_summary.csv","w",newline="",encoding="utf-8-sig") as f:
