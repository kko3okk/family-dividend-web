...
V=[
 ("J5","5檔x20% 上市（無產業層）",dict(swing=False,maxpos=5,pospct=0.20)),
 ("J5I","5檔x20% 上市＋產業層（排末段、主軸優先）",dict(swing=False,maxpos=5,pospct=0.20,industry=True)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/jan_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/jan_summary.csv","w",newline="",encoding="utf-8-sig") as f:
