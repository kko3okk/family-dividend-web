...
V=[
 ("T4","八行：5檔x20%",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.20)),
 ("R6","對照：6檔x16.7% 全給營收路徑",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=6,pospct=0.167)),
 ("R9","九行：5檔x16.7%＋報價位16.7%",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167)),
 ("R9b","九行：5檔x20%＋報價位20%（現金不足時報價位縮小）",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.20,rule9=0.20)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/r9_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
