...
START=200_000_000; FEE_B=0.001425; FEE_S=0.004425
    ...
    for i in range(N):
        ...
        if a and t60 and taiex[i]>=t60 and not block:
            ...
            for yoy,c,name,cum,sub,state in cand:
                ...
                v20=[vol[c][j] for j in range(max(0,i-19),i+1) if vol[c][j]]; v60=[vol[c][j] for j in range(max(0,i-59),i+1) if vol[c][j]]
                vr=(sum(v20)/len(v20))/(sum(v60)/len(v60)) if v20 and v60 else 0
                trades.append([d,c,name,f"進場YoY{yoy:.0f}% {sub}/{state} 累計{cum:.0f} 乖離{(px-m20)/m20*100:.1f} 量比{vr:.2f} 大盤乖離{(taiex[i]/ma(taiex,i,60)-1)*100:.1f} 名次{cand.index((yoy,c,name,cum,sub,state))+1}",sh,px,""])
...
V=[
 ("ALL","不限檔數，每檔100萬，全部符合條件都買",dict(swing=False,maxpos=999,pospct=0.005,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/all_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/all_summary.csv","w",newline="",encoding="utf-8-sig") as f:
