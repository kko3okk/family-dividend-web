...
MAXPOS=12; POSPCT=0.10; YOY_MIN=30; CYC_YOY=int(__import__("os").environ.get("CYC_YOY","15")); YOY_MAX=300
...
# 子產業同步加速判定
def sub_sync(ym,pym,N=3):
    ok={}
    for k,cs in SUB.items():
        n=0
        for c in cs:
            r=rev.get(ym,{}).get(c); p=rev.get(pym,{}).get(c)
            if r and p and r["yoy"] is not None and p["yoy"] is not None and r["yoy"]>0 and r["yoy"]>p["yoy"]+5: n+=1
        ok[k]=n>=N
    return ok
CYC_ON=True
SYNC={seq[k]:sub_sync(seq[k],seq[k-1]) for k in range(1,len(seq))}
...
for k in range(1,len(seq)):
    ...
    for c,r in rev[ym].items():
        ...
        p=rev[pym].get(c)
        cyc = CYC_ON and SYNC.get(ym,{}).get(code2sub.get(c,"—"),False) and r["yoy"] is not None and r["yoy"]>=CYC_YOY and c in code2sub
        std = r["yoy"] is not None and (YOY_MIN<=r["yoy"]<=YOY_MAX) and p and p["yoy"] is not None and p["yoy"]>=YOY_MIN
        if not (std or cyc): continue
        if not std: r=dict(r); r["note"]=(r["note"] or "")+"[循環路徑]"
        if ONETIME.search(r["note"] or ""): continue
        sub=code2sub.get(c,"—"); state=IND.get(ym,{}).get(sub,"無") if sub!="—" else "無"
        lst.append((r["yoy"] if std else r["yoy"]+1000,c,r["name"],(r.get("cum") or 0),sub,("循環" if not std else state)))
...
def run(opt):
    cash=START; pos={}; trades=[]; equity=[]; bought=set(); last_list=None; cooldown={}; loss_days=[]; last_entry=-999
    for i in range(N):
        d=dates[i]
        for c in list(pos):
        ...
        t60=ma(taiex,i,60)
        if opt.get("pv") and a and t60 and taiex[i]>=t60 and not block:
            ym=a[0]; extra=[]
            for c in code2sub:
                if c in pos or c in bought or c in fin: continue
                sv=close[c]; px=sv[i]
                if not px or i<60: continue
                hi=max(v for v in sv[i-60:i] if v) if any(sv[i-60:i]) else None
                if not hi or px<hi: continue                       # 創60日新高
                v20=[vol[c][j] for j in range(i-19,i+1) if vol[c][j]]; v60=[vol[c][j] for j in range(i-59,i+1) if vol[c][j]]
                if not v20 or not v60 or sum(v20)/len(v20) < opt["pv"]*sum(v60)/len(v60): continue   # 量增
                r=rev.get(ym,{}).get(c)
                if opt.get("pv_yoy") is not None and (not r or r["yoy"] is None or r["yoy"]<opt["pv_yoy"]): continue
                if opt.get("pv_cum") is not None and (not r or r.get("cum") is None or r["cum"]<opt["pv_cum"]): continue
                yy=r["yoy"] if r and r["yoy"] is not None else 0
                extra.append((yy,c,(r["name"] if r else c),(r.get("cum") if r else 0) or 0,code2sub.get(c,"—"),"價量"))
            a=(a[0], list(a[1])+extra)
        if a and t60 and taiex[i]>=t60 and not block:
            ...
            if opt.get("pv_first"): cand.sort(key=lambda t:(0 if t[5]=="價量" else 1, -t[0]))
            seen=set(); cand=[t for t in cand if not (t[1] in seen or seen.add(t[1]))]
            for yoy,c,name,cum,sub,state in cand:
                ...
                if state!="循環" and opt.get("cum_min") is not None and cum < opt["cum_min"]: continue
                if opt.get("in_map") and sub=="—": continue
                if opt.get("yoy_max") and yoy>opt["yoy_max"] and state!="循環": continue
...
V=[
 ("T4","基準",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
 ("Y1","＋循環路徑（同子產業≥3家同步加速、個股YoY≥15%），循環優先排序",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
 ("Y2","同上，不限檔數每檔100萬（看買到什麼）",dict(swing=False,maxpos=999,pospct=0.005,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/cyc_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/cyc_summary.csv","w",newline="",encoding="utf-8-sig") as f:
