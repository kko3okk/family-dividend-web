...
V=[
 ("T4","基準 U1+T4",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
 ("C1","虧損出清後10日不進場",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,loss_cool=10)),
 ("C2","虧損出清後20日不進場",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,loss_cool=20)),
 ("C3","15日內2次虧損出清→暫停進場20日",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,breaker=(2,15,20))),
 ("C4","大盤60MA乖離>8%不進場",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,taiex_bias=0.08)),
 ("C5","每次進場間隔≥10日",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,entry_gap=10)),
 ("C6","只在每月11–15日進場",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,monthly_window=True)),
 ("C7","C1＋C4",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,loss_cool=10,taiex_bias=0.08)),
 ("C8","C3＋C4",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,breaker=(2,15,20),taiex_bias=0.08)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/cool_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/cool_summary.csv","w",newline="",encoding="utf-8-sig") as f:
