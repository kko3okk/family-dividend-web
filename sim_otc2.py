...
V=[
 ("L11","十一行，只上市",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,bottom=(8,3))),
 ("L11O","十一行，上市＋上櫃（欄位修正版）",dict(swing=False,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,maxpos=5,pospct=0.167,rule9=0.167,no_mature=True,bottom=(8,3),otc=True)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/otc2_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/otc2_summary.csv","w",newline="",encoding="utf-8-sig") as f:
