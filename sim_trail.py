...
V=[
 ("U1","基準：只看60MA",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True)),
 ("T1","獲利≥50%後 高點回落15%出清",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.15,gain_min=0.5)),
 ("T2","獲利≥100%後 高點回落15%出清",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.15,gain_min=1.0)),
 ("T3","獲利≥100%後 高點回落20%出清",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.20,gain_min=1.0)),
 ("T4","獲利≥100%後 高點回落25%出清",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
 ("T5","獲利≥100%後 高點回落15%減半，其餘看60MA",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.15,gain_min=1.0,half=True)),
 ("T6","獲利≥50%後 高點回落20%減半",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.20,gain_min=0.5,half=True)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/trail_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/trail_summary.csv","w",newline="",encoding="utf-8-sig") as f:
