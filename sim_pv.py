...
V=[
 ("T4","基準：只有營收路徑",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0)),
 ("P1","＋價量路徑（60日新高、20日均量≥1.5x60日均量），不看營收",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,pv=1.5)),
 ("P2","＋價量路徑，且當月YoY≥15%",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,pv=1.5,pv_yoy=15)),
 ("P3","＋價量路徑，且累計YoY≥15%",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,pv=1.5,pv_cum=15)),
 ("P4","P2 且量比≥2.0",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,pv=2.0,pv_yoy=15)),
 ("P5","P2 但價量路徑優先排序",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,pv=1.5,pv_yoy=15,pv_first=True)),
 ("P6","P4＋C3 兩次虧損暫停",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30,in_map=True,trail=0.25,gain_min=1.0,pv=2.0,pv_yoy=15,breaker=(2,15,20))),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/pv_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/pv_summary.csv","w",newline="",encoding="utf-8-sig") as f:
