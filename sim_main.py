...
V=[
 ("J5","基準 5檔x20%",dict(swing=False,maxpos=5,pospct=0.20)),
 ("M1","主流＝20日均成交額≥5億",dict(swing=False,maxpos=5,pospct=0.20,turnover=5e8)),
 ("M2","主流＝20日均成交額≥10億",dict(swing=False,maxpos=5,pospct=0.20,turnover=1e9)),
 ("M3","主流＝累計YoY≥30（排低基期）",dict(swing=False,maxpos=5,pospct=0.20,cum_min=30)),
 ("M4","主流＝在子產業對照表內",dict(swing=False,maxpos=5,pospct=0.20,in_map=True)),
 ("M5","M1＋M3",dict(swing=False,maxpos=5,pospct=0.20,turnover=5e8,cum_min=30)),
 ("M6","M1＋YoY上限150%",dict(swing=False,maxpos=5,pospct=0.20,turnover=5e8,yoy_max=150)),
 ("M7","M5＋3檔x33%",dict(swing=False,maxpos=3,pospct=0.333,turnover=5e8,cum_min=30)),
 ("M8","M1＋M4",dict(swing=False,maxpos=5,pospct=0.20,turnover=5e8,in_map=True)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/main_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/main_summary.csv","w",newline="",encoding="utf-8-sig") as f:
