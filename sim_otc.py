...
OTC_DIR="data/prices_otc"; os.makedirs(OTC_DIR,exist_ok=True)
HDR={"User-Agent":"Mozilla/5.0"}
def _get(url):
    with urllib.request.urlopen(urllib.request.Request(url,headers=HDR),timeout=60) as r: return r.read().decode("utf-8","ignore")
def _f(x):
    try: return float(str(x).replace(",","").replace("%",""))
    except Exception: return None
def fetch_otc(dd):
    cands=[f"https://www.tpex.org.tw/www/zh-tw/afterTrading/otc?date={dd.strftime('%Y/%m/%d')}&type=EW&response=json",
           f"https://www.tpex.org.tw/web/stock/aftertrading/otc_quotes_no1430/stk_wn1430_result.php?l=zh-tw&d={dd.year-1911}/{dd.month:02d}/{dd.day:02d}&se=EW&o=json"]
    for u in cands:
        try: j=json.loads(_get(u))
        except Exception: continue
        rr=[]
        if isinstance(j,dict) and "tables" in j:
            for tb in j["tables"]: rr+=tb.get("data",[])
        if isinstance(j,dict): rr+=j.get("aaData",[])
        px={}
        for r in rr:
            try:
                c=str(r[0]).strip()
                if re.fullmatch(r"\d{4}",c) and _f(r[2]): px[c]=[_f(r[2]), _f(r[8]) or 0]
...
while _d<=_end:
    ...
    if os.path.exists(p):
        otc_days.update(json.load(open(p,encoding="utf-8")))
    else:
        month={}; dd=_d
...
print("上櫃交易日",len(otc_days),flush=True)
...
for k in range(1,len(seq)):
    ...
    for c,r in rev[ym].items():
        ...
        lst.append((r["yoy"],c,r["name"],(r.get("cum") or 0),r["market"]))
...
def run(opt):
    ...
    for i in range(N):
        ...
        if a and t60 and taiex[i]>=t60:
            ...
            for yoy,c,name,cum,mkt in cand:
...
V=[
 ("J5","5檔x20% 上市",dict(swing=False,maxpos=5,pospct=0.20)),
 ("J5O","5檔x20% 上市+上櫃",dict(swing=False,maxpos=5,pospct=0.20,otc=True)),
 ("J6O","6檔x16.7% 上市+上櫃",dict(swing=False,maxpos=6,pospct=0.167,otc=True)),
 ("J8O","8檔x12.5% 上市+上櫃",dict(swing=False,maxpos=8,pospct=0.125,otc=True)),
 ("J4O","4檔x25% 上市+上櫃",dict(swing=False,maxpos=4,pospct=0.25,otc=True)),
]
rows=[]
for k,desc,opt in V:
    m,tr=run(opt)
    rows.append([k,desc,f"{m['ret']:+.2f}",f"{m['mdd']:.2f}",m["trades"],m["ent"],f"{m['win']:.0f}",m["hold"],f"{m['final']:,.0f}"])
    with open(f"{OUT}/otc_{k}_trades.csv","w",newline="",encoding="utf-8-sig") as f:
...
with open(f"{OUT}/optimize_otc.csv","w",newline="",encoding="utf-8-sig") as f:
